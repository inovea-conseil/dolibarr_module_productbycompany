...
llxHeader('', $langs->trans('ProductByCompanyList'), '', '');

//$type = GETPOST('type');
//if (empty($user->rights->productbycompany->all->read)) $type = 'mine';
...
$sql.= ' FROM '.MAIN_DB_PREFIX.'productbycompany t ';

if (!empty($object->isextrafieldmanaged))
{
    $sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'productbycompany_extrafields et ON (et.fk_object = t.rowid)';
}
...
echo $r->render($sql, array(
    ...
    ,'list' => array(
        'title' => $langs->trans('ProductByCompanyList')
	...
	,'search' => array(
		...
		,'status' => array('search_type' => ProductByCompany::$TStatus, 'to_translate' => true) // select html, la clé = le status de l'objet, 'to_translate' à true si nécessaire
	)
	,'translate' => array()
	,'hide' => array(
