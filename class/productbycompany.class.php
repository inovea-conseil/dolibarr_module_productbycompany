...
class ProductByCompany extends SeedObject
{
	...
	public static $TStatus = array(
		self::STATUS_CANCELED => 'ProductByCompanyStatusShortCanceled'
		,self::STATUS_DRAFT => 'ProductByCompanyStatusShortDraft'
		,self::STATUS_VALIDATED => 'ProductByCompanyStatusShortValidated'
//		,self::STATUS_REFUSED => 'ProductByCompanyStatusShortRefused'
//		,self::STATUS_ACCEPTED => 'ProductByCompanyStatusShortAccepted'
	);

	/** @var string $table_element Table name in SQL */
	public $table_element = 'productbycompany';
...
//        ),
    ...
    private function getNextRef()
    {
		...
		$mask = !empty($conf->global->PRODUCTBYCOMPANY_REF_MASK) ? $conf->global->PRODUCTBYCOMPANY_REF_MASK : 'MM{yy}{mm}-{0000}';
		$ref = get_next_value($db, $mask, 'productbycompany', 'ref');
...
//        $picto='productbycompany@productbycompany';
    ...
    public static function LibStatut($status, $mode)
    {
        ...
        if ($status==self::STATUS_CANCELED) { $statusType='status9'; $statusLabel=$langs->trans('ProductByCompanyStatusCancel'); $statusLabelShort=$langs->trans('ProductByCompanyStatusShortCancel'); }
        elseif ($status==self::STATUS_DRAFT) { $statusType='status0'; $statusLabel=$langs->trans('ProductByCompanyStatusDraft'); $statusLabelShort=$langs->trans('ProductByCompanyStatusShortDraft'); }
        elseif ($status==self::STATUS_VALIDATED) { $statusType='status1'; $statusLabel=$langs->trans('ProductByCompanyStatusValidated'); $statusLabelShort=$langs->trans('ProductByCompanyStatusShortValidate'); }
        elseif ($status==self::STATUS_REFUSED) { $statusType='status5'; $statusLabel=$langs->trans('ProductByCompanyStatusRefused'); $statusLabelShort=$langs->trans('ProductByCompanyStatusShortRefused'); }
        elseif ($status==self::STATUS_ACCEPTED) { $statusType='status6'; $statusLabel=$langs->trans('ProductByCompanyStatusAccepted'); $statusLabelShort=$langs->trans('ProductByCompanyStatusShortAccepted'); }
